...
class Admin(tk.Tk):
    def __init__(self):
        ...
        label_entry_titulo.grid(row=0,column=0, padx=20, sticky="w", pady=(30,10), columnspan=6)
        
        self.titulo = ""
        self.titulo_entry = tk.StringVar()
        entry_titulo = tk.Entry(frame_izq, textvariable=self.titulo_entry, font=STDFONT,bd=0)
        entry_titulo.grid(row=1, ipady=5,padx=20,columnspan=7, sticky=tk.EW)
        ...
        btn_mPelicula=ttk.Button(frame_der, text="Modificar Pelicula " , command=self.guardar, style='flat.TButton',padding=20).grid(row=3,column=0, pady=(20,7),padx=20,sticky='WE')
